...
offset=audit.get('board_offset_mm',[0,0])
delta=p.VECTOR2I(p.FromMM(offset[0]),p.FromMM(offset[1]))
for group in [list(old.GetFootprints()),list(old.GetTracks()),list(old.Zones()),list(old.GetDrawings())]:
...
def pads(b,kind):
 def manufacturing_layers(f,pad):
  layers=pad.GetLayerSet()
  # Printed antennas intentionally have no solder paste after the A2 review.
  if f.GetReference().startswith('U$'): layers.RemoveLayer(p.F_Paste)
  return layers.FmtBin()
 return sorted([[f.GetReference(),pad.GetNumber(),pad.GetNetname(),xy(pad.GetPosition()),xy(pad.GetSize()),xy(pad.GetDrillSize()),manufacturing_layers(f,pad)]
...
assert all(not pad.GetLayerSet().Contains(p.F_Paste) for f in new.GetFootprints() if f.GetReference().startswith('U$') for pad in f.Pads())
...
result={'board_offset_mm':offset,'geometry_comparison_frame':'source translated by board_offset_mm','source_hashes_unchanged':True,'datasheet_copy_identical':True,'antenna_pad_geometry_layers_and_nets_identical':True,
 ...
 'drc_disabled_checks':drc['ignored_checks'],'antenna_paste_removed_intentionally':True,
